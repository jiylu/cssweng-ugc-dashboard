...
export function ApiCreateWrittenAssetDraft() {
  return applyDecorators(
    ApiOperation({
      ...
      description: 'Creates a new draft version for a specific written asset. Requires the written asset public ID and the text content of the draft.',
    }),
    ApiBody({ type: CreateWrittenAssetDraftDto }),
    ApiResponse({ 
      status: 201, 
      description: 'Draft created successfully. Returns the created draft details.',
      schema: { example: exampleWrittenAssetDraftResponse }
    }),
    ApiResponse({ status: 400, description: 'Invalid request body payload.' }),
    ApiResponse({ status: 404, description: 'Written asset not found. The provided writtenAssetPublicId does not exist.' }),
...
export function ApiFindWrittenAssetDraft() {
  return applyDecorators(
    ApiOperation({
      ...
      description: 'Retrieves a single written asset draft using its unique public ID.'
    }),
    ApiParam({ name: 'publicId', type: String, required: true, example: 'x21E9dlf0F' }),
    ApiResponse({ 
      status: 200, 
      description: 'Draft retrieved successfully.',
      schema: { example: exampleWrittenAssetDraftResponse }
    }),
    ApiResponse({ status: 404, description: 'Draft not found. The given public ID does not match any existing draft.' }),
    ApiResponse({ status: 500, description: 'Internal server error.' }),
  );
}
...
export function ApiFindWrittenAssetDraftsForAsset() {
  return applyDecorators(
    ApiOperation({
      ...
      description: 'Retrieves all drafts associated with a specific written asset. Useful for version history or recovering previous drafts.'
    }),
    ApiQuery({ name: 'writtenAssetPublicId', required: true, type: String, example: 'pub-written-1' }),
    ApiResponse({ 
      status: 200, 
      description: 'Drafts retrieved successfully. Returns an array of drafts.',
      schema: { example: [exampleWrittenAssetDraftResponse] }
    }),
    ApiResponse({ status: 400, description: 'Invalid query parameters (e.g., missing writtenAssetPublicId).' }),
...
export function ApiUpdateWrittenAssetDraft() {
  return applyDecorators(
    ApiOperation({
      ...
      description: 'Updates the content of an existing written asset draft. Only provided fields are updated.'
    }),
    ApiParam({ name: 'publicId', type: String, required: true, example: 'x21E9dlf0F' }),
...
export function ApiDeleteWrittenAssetDraft() {
  return applyDecorators(
    ...
    ApiResponse({ 
      status: 200, 
      description: 'Draft deleted successfully. Returns the details of the deleted draft.',
      schema: { example: exampleWrittenAssetDraftResponse }
    }),
    ApiResponse({ status: 404, description: 'Draft not found.' }),
    ApiResponse({ status: 500, description: 'Internal server error.' }),
